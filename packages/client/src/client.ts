  ...
  if (selectedComponentForA11y) {
    ...
    if (instance) {
      ...
      resultsDiv.innerHTML =
        '<div class="wc-no-components">Click an accessibility badge in the Components tab to view details</div>';
    }
  } else {
    resultsDiv.innerHTML =
      '<div class="wc-no-components">Click an accessibility badge in the Components tab to view details</div>';
...
function watchForChanges(panel: HTMLDivElement): void {
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    const hasCustomElementChanges = mutations.some((mutation) => {
      ...
      if (
        devtoolsContent &&
        (mutation.target === devtoolsContent || devtoolsContent.contains(mutation.target as Node))
      ) {
        return false;
      }
